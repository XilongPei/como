...
{
    como::HashMapCache<Key*, std::string*> cache;

    Key *pk1 = new Key(1), *pk2 = new Key(2), *pk3 = new Key(3);
    std::string value1 = "Value1", value2 = "Value2";
    std::string *pv1 = &value1, *pv2 = &value2;
    EXPECT_EQ(cache.Put(pk1, pv1, 10), 0);
    EXPECT_EQ(cache.Put(pk2, pv2, 10), 0);

    EXPECT_EQ(cache.Get(pk1), pv1);
    EXPECT_EQ(cache.Get(pk2), pv2);
    EXPECT_EQ(cache.Get(pk3), std::string(0));
}
