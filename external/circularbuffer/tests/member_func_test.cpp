...
TEST_F(CircularBufferTest, PopFrontTest) {
    ...
    EXPECT_FALSE(test_buff.pop_front());
}
...
TEST_F(CircularBufferTest, BackTest) {
    ...
    std::string str = test_buff.back();
    EXPECT_TRUE(str.empty() || test_buff.IsElement(&str));
}
...
TEST_F(CircularBufferTest, AtFunctionTest) {
    ...
    std::string str = test_buff.at(TEST_BUFFER_SIZE) = "test_string";
    EXPECT_TRUE(str.empty() || test_buff.IsElement(&str));

    //test out of size
    test_buff.pop_front();
    str = test_buff.at(TEST_BUFFER_SIZE - 1);
    EXPECT_TRUE(str.empty() || test_buff.IsElement(&str));
}
...
TEST_F(CircularBufferTest, AccessOperatorTest) {
    ...
    //test out of bounds
    std::string str = test_buff[TEST_BUFFER_SIZE];
    EXPECT_TRUE(str.empty() || test_buff.IsElement(&str));

    //test out of size
    test_buff.pop_front();
    str = test_buff[TEST_BUFFER_SIZE - 1] = "test_string";
    EXPECT_TRUE(str.empty() || test_buff.IsElement(&str));
}
