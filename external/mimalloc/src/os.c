...
static void* mi_unix_mmap(void* addr, size_t size, size_t try_alignment,
           int protect_flags, bool large_only, bool allow_large, bool* is_large)
{
    void* p = NULL;
...
#define MAP_NORESERVE  0
            ...
            if (large_only || lflags != flags) {
                ...
                *is_large = true;
                ...
                if (p == NULL && (lflags & MAP_HUGE_1GB) != 0) {
                    ...
                    _mi_warning_message("unable to allocate huge (1GiB) page, "
                                "trying large (2MiB) pages instead (error %i)\n", errno);
    ...
    if (p == NULL) {
        _mi_warning_message("unable to allocate OS memory (%zu bytes, error "
                            "code: %i, address: %p, large only: %d, allow large: %d)\n",
...
/**
 * Return a 4MiB aligned address that is probably available. If this returns
 * NULL, the OS will determine the address but on some OS's that may not be
 * properly aligned which can be more costly as it needs to be adjusted afterwards.
 * For a size > 1GiB this always returns NULL in order to guarantee good ASLR
 * randomization; (otherwise an initial large allocation of say 2TiB has a 50%
 * chance to include (known) addresses in the middle of the 2TiB - 6TiB address
 * range (see issue #372))
 */

// 2TiB start
#define KK_HINT_BASE ((uintptr_t)2 << 40)
// upto 6TiB   (since before win8 there is "only" 8TiB available to processes)
#define KK_HINT_AREA ((uintptr_t)4 << 40)
// wrap after 30TiB (area after 32TiB is used for huge OS pages)
#define KK_HINT_MAX  ((uintptr_t)30 << 40)
...
static void* mi_os_get_aligned_hint(size_t try_alignment, size_t size)
{
    ...
    if ((size % MI_SEGMENT_SIZE) != 0)
        return NULL;
    if (size > 1 * MI_GiB) {
    // guarantee the chance of fixed valid address is at most 1/(KK_HINT_AREA / 1<<30) = 1/4096.
        return NULL;
    }
#if (MI_SECURE>0)
    // put in `MI_SEGMENT_SIZE` virtual gaps between hinted blocks; this splits
    // VLA's but increases guarded areas.
    size += MI_SEGMENT_SIZE;
#endif

    uintptr_t hint = mi_atomic_add_acq_rel(&aligned_base, size);
    if (hint == 0 || hint > KK_HINT_MAX) {
    // wrap or initialize
...
// security: randomize start of aligned allocations unless in debug mode
        ...
        init = init + ((MI_SEGMENT_SIZE * ((r >> 17) & 0xFFFFF)) % KK_HINT_AREA);
#endif
        uintptr_t expected = hint + size;
        mi_atomic_cas_strong_acq_rel(&aligned_base, &expected, init);

        // this may still give 0 or > KK_HINT_MAX but that is ok, it is a hint after all
...
static void* mi_os_mem_alloc(size_t size, size_t try_alignment, bool commit,
                            bool allow_large, bool* is_large, mi_stats_t* stats)
{
...
void* _mi_os_alloc_aligned(size_t size, size_t alignment, bool commit,
                                             bool* large, mi_stats_t* tld_stats)
{
    MI_UNUSED(tld_stats);
    ...
    return mi_os_mem_alloc_aligned(size, alignment, commit, allow_large,
            (large != NULL ? large : &allow_large), &_mi_stats_main /*tld->stats*/ );
}
...
static void* mi_os_page_align_areax(bool conservative, void* addr, size_t size, size_t* newsize)
{
    ...
    void* start = (conservative
                    ? mi_align_up_ptr(addr, _mi_os_page_size())
                    : mi_align_down_ptr(addr, _mi_os_page_size()));

    void* end = (conservative
...
static bool mi_os_commitx(void* addr, size_t size, bool commit, bool conservative,
                                                bool* is_zero, mi_stats_t* stats)
{
    ...
    if (!commit) {
        ...
        void* p = mmap(start, csize, PROT_NONE, (MAP_FIXED | MAP_PRIVATE |
...
bool _mi_os_unreset(void* addr, size_t size, bool* is_zero, mi_stats_t* tld_stats)
{
    ...
    // re-commit it (conservatively!)
        return mi_os_commit_unreset(addr, size, is_zero, stats);
    }
    else {
        *is_zero = false;
...
void* _mi_os_alloc_huge_os_pages(size_t pages, int numa_node, mi_msecs_t max_msecs,
                                            size_t* pages_reserved, size_t* psize)
{
    if (psize != NULL)
        *psize = 0;
...
static size_t mi_os_numa_node_countx(void)
{
    ...
    static size_t mi_os_numa_node_countx(void)
    {
        ...
        for (node = 0; node < 256; node++) {
            // enumerate node entries
            // -- todo: it there a more efficient way to do this? (but ensure there is no allocation)
