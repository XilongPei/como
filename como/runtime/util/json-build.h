...
// Ref: https://gitee.com/tjopenlab/json-build
...
#define jsonb_reset(builder) ((builder)->pos = 0)
     ...
     */
    JSONB_API void jsonb_init(jsonb *builder, char *buf, size_t bufsize);
     ...
     */
    JSONB_API jsonbcode jsonb_object(jsonb *builder);
    ...
    JSONB_API jsonbcode jsonb_key(
        jsonb *builder, const char key[], size_t len);
     ...
     * @return @ref jsonbcode value
     */
    JSONB_API jsonbcode jsonb_array(jsonb *builder);
     ...
     * @return @ref jsonbcode value
     */
    JSONB_API jsonbcode jsonb_null(jsonb *builder);
    ...
    JSONB_API jsonbcode jsonb_string(
        jsonb *builder, const char str[], size_t len);
...
#define BUFFER_COPY(b, value, len, _pos, buf, bufsize) \
    ...
    JSONB_API void
    jsonb_init(jsonb *b, char *buf, size_t bufsize)
    {
    ...
    jsonb_object(jsonb *b)
    {
        ...
        {
        case JSONB_ARRAY_NEXT_VALUE_OR_CLOSE:
            BUFFER_COPY_CHAR(b, ',', pos, b->buf, b->bufsize);
            /* fall-through */
        case JSONB_ARRAY_VALUE_OR_CLOSE:
    ...
    JSONB_API jsonbcode
    jsonb_key(jsonb *b, const char key[], size_t len)
    {
        size_t pos = 0;
        switch (*b->top)
        {
        ...
        case JSONB_OBJECT_KEY_OR_CLOSE:
        {
            enum jsonbcode ret;
            BUFFER_COPY_CHAR(b, '"', pos, b->buf, b->bufsize);
            ret = _jsonb_escape(&pos, b->buf + b->pos, b->bufsize - b->pos, key, len);
            if (ret != JSONB_OK)
                return ret;
            BUFFER_COPY(b, "\":", 2, pos, b->buf, b->bufsize);
    ...
    JSONB_API jsonbcode
    jsonb_token(
        jsonb *b, const char token[], size_t len)
    {
        ...
        BUFFER_COPY(b, token, len, pos, b->buf, b->bufsize);
    ...
    JSONB_API jsonbcode
    jsonb_bool(jsonb *b, int boolean)
    {
        if (boolean)
            return jsonb_token(b, "true", 4);
        return jsonb_token(b, "false", 5);
    }

      JSONB_API jsonbcode
    jsonb_null(jsonb *b)
    {
        return jsonb_token(b, "null", 4);
    }

    JSONB_API jsonbcode
    jsonb_string(
        jsonb *b, const char str[], size_t len)
    {
        enum jsonbstate next_state;
        ...
        BUFFER_COPY_CHAR(b, '"', pos, b->buf, b->bufsize);
        ret = _jsonb_escape(&pos, b->buf + b->pos, b->bufsize - b->pos, str, len);
        if (ret != JSONB_OK)
            return ret;
        BUFFER_COPY_CHAR(b, '"', pos, b->buf, b->bufsize);
        STACK_HEAD(b, next_state);
    ...
    JSONB_API jsonbcode
    jsonb_number(jsonb *b, double number)
    {
        char token[32];
        long len = sprintf(token, "%.17G", number);
        if (len < 0)
            return JSONB_ERROR_INPUT;
        return jsonb_token(b, token, len);
    }
